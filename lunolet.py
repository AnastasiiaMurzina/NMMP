...
def main(m0_fluel, u):
    ...
    v=0
    arr_h=[h] # храненим высоты
    arr_v=[v] # для скорости
    arr_t=[0]
    print("Необходимо посадить лунолёт с высоты ", h, " метров. С запасом топлива ", m0_fluel, " кг.")
    print("Вводите данные в форматe 'dm dt' или 'dm,dt', разделив запятой или пробелом.")
    while(True):
        print("\n")
        string=(input("Введите массу (кг) и время выброса топлива (с):")).replace(",", " ").split() # ждём два числа через , или пробел
        ...
        if m0_fluel!=0:# если есть топливо
            if abs(overheight(M, dm, dt, u_cur)) <= 5:  # проверяем на перегрузку
                ...
                h = high(M, h, v, 0, 10, 0) 
                v = velocity(M, v, 0, 10, 0)  # топливо не выбрасывается в течении 10 секунд
                arr_t.append(arr_t[-1] + 10)  # добавляем 10 к времени
            arr_h.append(max(h, 0))  # всегда выводим
            arr_v.append(v) 
